...
test.describe("Invalid Single Featured Product Checkout Confirmation Tests - No Singular Input", () => {
    ...
    // Test 029j -> invalid single featured product ("Epson EF-100 Smart Portable Projector") check out confirmation test (as a registered user) - no credit card expiration year
    test("Invalid Single Featured Product Checkout Confirmation Test (as a registered user) - No Credit Card Exp Year", async ({page}) => {
        //launch the page
        await page.goto('/');
        //navigate to signup page test
        await testMethods.navigateToSignUpPageTest(page);
        ...
        //add single featured product ("Epson EF-100 Smart Portable Projector") to check out test (as a registered user)
        await testMethods.addProductToCheckoutTest(page);
        //invalid single featured product ("Epson EF-100 Smart Portable Projector") check out confirmation test (as a registered user) - no credit card expiration year
        await testMethods.invalidProductCheckoutConfirmationNoCreditCardExpYearTest(page);
    });

    // Test 029k -> invalid single featured product ("Epson EF-100 Smart Portable Projector") check out confirmation test (as a registered user) - no credit card CVV number (after click "Place order", exp month disappears, triggering the error as well)
    test("Invalid Single Featured Product Checkout Confirmation Test (as a registered user) - No Credit Card CVV Number", async ({page}) => {
        //launch the page
        await page.goto('/');
        //navigate to signup page test
        await testMethods.navigateToSignUpPageTest(page);
        ...
        //add single featured product ("Epson EF-100 Smart Portable Projector") to check out test (as a registered user)
        await testMethods.addProductToCheckoutTest(page);
        //invalid single featured product ("Epson EF-100 Smart Portable Projector") check out confirmation test (as a registered user) - no credit card CVV number
        await testMethods.invalidProductCheckoutConfirmationNoCreditCardCVVTest(page);
    });
