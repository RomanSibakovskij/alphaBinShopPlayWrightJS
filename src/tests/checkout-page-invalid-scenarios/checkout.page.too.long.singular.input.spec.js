...
test.describe("Invalid Single Featured Product Checkout Confirmation Tests - Too Long Singular Input", () => {
    ...
    // Test 029aj -> invalid single featured product ("Epson EF-100 Smart Portable Projector") check out confirmation test (as a registered user) - too long credit card CVV number (4 digits) (the error was triggered but the input form doesn't allow 4-digit input)
    test("Invalid Single Featured Product Checkout Confirmation Test (as a registered user) - Too Long Credit Card CVV Number", async ({page}) => {
        //launch the page
        await page.goto('/');
        //navigate to signup page test
        await testMethods.navigateToSignUpPageTest(page);
        ...
        //add single featured product ("Epson EF-100 Smart Portable Projector") to check out test (as a registered user)
        await testMethods.addProductToCheckoutTest(page);
        //invalid single featured product ("Epson EF-100 Smart Portable Projector") check out confirmation test (as a registered user) - too long credit card CVV number
        await testMethods.invalidProductCheckoutConfirmationTooLongCreditCardCVVNumberTest(page);
    });
